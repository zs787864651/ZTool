...
public class PasswordDBHelper extends SQLiteOpenHelper {
        ...
        if (null == sDBHelper) {
            synchronized (PasswordDBHelper.class) {
                if (null == sDBHelper) {
                    ...
                    //sDBHelper.getDBPathReal(context);
                }
            }
        }
        return sDBHelper;
    ...
    /**获取数据库路径**/
    private void getDBPathReal(Context context){
        String tmp = context.getDatabasePath(DATABASE_NAME).getPath();
        Toast.makeText(context,"geek.db address："+tmp,Toast.LENGTH_LONG).show();
        Log.e("tmp","geek.db address："+tmp);
    }
